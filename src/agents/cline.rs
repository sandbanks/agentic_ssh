...
//!
//! Handles registration of the agentic_ssh MCP server in Cline's
...
impl AgentIntegration for ClineIntegration {
    ...
    fn uninstall(&self, ctx: &InstallContext) -> Result<()> {
        ...
        eprintln!();
        eprintln!("Uninstall complete. AgenticSsh has been removed from Cline.");
...
/// Check Cline's `cline_mcp_settings.json` has agentic_ssh MCP server registered.
fn doctor_check_settings(dc: &mut DoctorCounters, home: &Path) {
    let settings_path = cline_ext_dir(home).join("settings/cline_mcp_settings.json");

    if !settings_path.exists() {
        dc.warn(&format!(
            "{} not found — run `agentic_ssh install --agent cline` if you use Cline",
            settings_path.display()
        ));
        return;
    }
    ...
    if server.and_then(|v| v.as_object()).is_some() {
        ...
        dc.fail(&format!(
            "MCP server NOT registered in {} — run `agentic_ssh install --agent cline`",
            settings_path.display()
        ));
    }
}
