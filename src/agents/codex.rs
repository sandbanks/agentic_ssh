...
//!
//! Handles registration of the agentic_ssh MCP server in Codex's config
...
impl AgentIntegration for CodexIntegration {
    ...
    fn install(&self, ctx: &InstallContext) -> Result<()> {
        ...
        eprintln!("  1. Start a new Codex session — agentic_ssh tools are now available");
        Ok(())
    }
    ...
    fn uninstall(&self, ctx: &InstallContext) -> Result<()> {
        ...
        eprintln!();
        eprintln!("Uninstall complete. AgenticSsh has been removed from Codex CLI.");
...
fn install_mcp_server(config_path: &Path, agentic_ssh_bin: &str) -> Result<()> {
    ...
    // Ensure [mcp_servers.agentic_ssh] exists
    let table = config
        .as_table_mut()
    ...
    // Auto-approve all agentic_ssh tools so Codex doesn't prompt for each one
...
/// Check config.toml has agentic_ssh registered.
fn doctor_check_config(dc: &mut DoctorCounters, config_path: &Path) {
    if !config_path.exists() {
        dc.warn(&format!(
            "{} not found — run `agentic_ssh install --agent codex` if you use Codex CLI",
            config_path.display()
        ));
        return;
    }
    ...
    if !has_server {
        dc.fail(&format!(
            "MCP server NOT registered in {} — run `agentic_ssh install --agent codex`",
            config_path.display()
        ));
        return;
    }
    ...
    if auto_count >= tools_len {
        ...
        dc.warn(&format!(
            "{auto_count}/{tools_len} tools auto-approved — run `agentic_ssh install --agent codex` to update"
        ));
    } else {
