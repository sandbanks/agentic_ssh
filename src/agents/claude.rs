...
//!
//! Handles registration of the agentic_ssh MCP server in Claude Code's config
...
fn install_local(ctx: &InstallContext, project: &Path) -> Result<()> {
    ...
    install_mcp_server(&mcp_json_path, &ctx.agentic_ssh_bin)?;
...
fn uninstall_global(ctx: &InstallContext) {
    ...
    eprintln!();
    eprintln!("Uninstall complete. AgenticSsh has been removed from Claude Code.");
...
fn uninstall_local(project: &Path) {
    ...
    eprintln!();
    eprintln!(
        "Removed agentic_ssh from this project ({}).",
        project.display()
    );
}
...
fn install_mcp_server(claude_json_path: &Path, agentic_ssh_bin: &str) -> Result<()> {
    ...
    claude_json["mcpServers"]["agentic_ssh"] = json!({
...
fn install_migrate_old_mcp(settings: &mut serde_json::Value, settings_path: &Path) {
    ...
    {
        if servers.remove("agentic_ssh").is_some() {
            if servers.is_empty() {
...
fn install_hook(settings: &mut serde_json::Value, agentic_ssh_bin: &str) {
    install_hook_inner(settings, agentic_ssh_bin, false);
}

/// Add all agentic_ssh hooks silently (for post-upgrade migration).
fn install_hook_quiet(settings: &mut serde_json::Value, agentic_ssh_bin: &str) {
    install_hook_inner(settings, agentic_ssh_bin, true);
}

fn install_hook_inner(settings: &mut serde_json::Value, agentic_ssh_bin: &str, quiet: bool) {
    install_single_hook(
        settings,
        "PreToolUse",
        agentic_ssh_bin,
    ...
    install_single_hook(settings, "Stop", agentic_ssh_bin, "hook-stop", None, quiet);
}
...
fn install_single_hook(
    ...
    let has_hook = hooks_arr
        .iter()
        .any(|h| hook_entry_command(h).is_some_and(|c| c.contains("agentic_ssh")));
...
/// Find the first agentic_ssh hook entry under an event and return
...
fn find_agentic_ssh_hook(
    ...
    arr.iter().find_map(|wrapper| {
        ...
        if !raw_command.contains("agentic_ssh") {
            return None;
        }
                    ...
                    .and_then(|v| v.as_object_mut())
                {
                    if servers.remove("agentic_ssh").is_some() {
                        if servers.is_empty() {
                            ...
                            eprintln!("\x1b[32m✔\x1b[0m Removed agentic_ssh from local .mcp.json (using global config only)");
                        }
                    }
                }
            }
...
/// Remove agentic_ssh entries from a local settings.local.json file.
fn clean_local_settings_file(project_path: &Path, local_settings_path: &Path) {
    ...
    {
        let before = arr.len();
        arr.retain(|v| v.as_str() != Some("agentic_ssh"));
        if arr.len() < before {
            modified = true;
        }
    }
    ...
    if is_empty {
        if std::fs::remove_file(local_settings_path).is_ok() {
            eprintln!(
                "\x1b[32m✔\x1b[0m Removed {} (agentic_ssh should only be in global config)",
        ...
        eprintln!(
            "\x1b[32m✔\x1b[0m Removed agentic_ssh entries from {} (should only be in global config)",
            local_settings_path.display()
        );
    }
}
...
fn uninstall_mcp_server(claude_json_path: &Path) {
    ...
    if servers.remove("agentic_ssh").is_none() {
        eprintln!("  No agentic_ssh MCP server in ~/.claude.json, skipping");
        return;
    }
    if servers.is_empty() {
...
/// Remove all agentic_ssh hooks. Returns true if modified.
...
/// Remove agentic_ssh entries from a single hook event. Returns true if modified.
                ...
                .is_some_and(|arr| {
                    arr.iter().any(|entry| {
                        entry
                            .get("command")
                            .and_then(|c| c.as_str())
                            .is_some_and(|c| c.contains("agentic_ssh"))
                    })
                })
        })
        .collect();
...
/// Remove agentic_ssh tool permissions. Returns true if modified.
fn uninstall_permissions(settings: &mut serde_json::Value) -> bool {
    ...
    let filtered: Vec<serde_json::Value> = arr
        ...
        .filter(|v| {
            !v.as_str()
                .is_some_and(|s| s.starts_with("mcp__agentic_ssh__"))
        })
        .collect();
    if filtered.len()
        >= settings["permissions"]["allow"]
    ...
    eprintln!("\x1b[32m✔\x1b[0m Removed agentic_ssh tool permissions");
    true
}

/// Remove agentic_ssh rules from CLAUDE.md.
fn uninstall_claude_md_rules(claude_md_path: &Path) {
    ...
    // Skip past any sub-headings that are part of our agentic_ssh rules block
        ...
        loop {
            match contents[search_from..].find("\n## ") {
                Some(pos) => {
                    ...
                    if heading_line.contains("agentic_ssh") {
...
fn doctor_check_claude_json(dc: &mut DoctorCounters, home: &Path) {
    ...
    if !claude_json_path.exists() {
        dc.fail("~/.claude.json not found — run `agentic_ssh install`");
        return;
    }
    ...
    let mcp_entry = &claude_json["mcpServers"]["agentic_ssh"];
    if !mcp_entry.is_object() {
        dc.fail("MCP server NOT registered in ~/.claude.json — run `agentic_ssh install`");
        return;
    }
    ...
    if args_ok {
        ...
        dc.fail("MCP server args missing \"serve\" — run `agentic_ssh install`");
    }
}
...
fn doctor_check_mcp_binary(dc: &mut DoctorCounters, mcp_entry: &serde_json::Value) {
    let Some(mcp_cmd) = mcp_entry["command"].as_str() else {
        dc.fail("MCP server entry missing \"command\" field — run `agentic_ssh install`");
        return;
    };
    let mcp_bin = Path::new(mcp_cmd);
    if !mcp_bin.exists() {
        dc.fail(&format!(
            "MCP binary not found: {mcp_cmd} — run `agentic_ssh install`"
        ));
        return;
    }
...
/// Auto-repairs missing hooks when a agentic_ssh binary can be determined.
fn doctor_check_settings_json(dc: &mut DoctorCounters, home: &Path) {
    ...
    if settings_path.exists() {
        if let Some(settings) = std::fs::read_to_string(&settings_path)
            ...
            .and_then(|c| serde_json::from_str::<serde_json::Value>(&c).ok())
        {
            if settings["mcpServers"]["agentic_ssh"].is_object() {
...
/// symbol-shaped greps to `agentic_ssh_search` / `agentic_ssh_signature_search` /
/// `agentic_ssh_callers`.
...
/// Find the matcher string currently installed on a agentic_ssh hook entry.
/// Returns `None` if the hook isn't installed or has no matcher field.
fn find_agentic_ssh_hook_matcher(settings: &serde_json::Value, event: &str) -> Option<String> {
    let arr = settings["hooks"][event].as_array()?;
    arr.iter().find_map(|wrapper| {
...
/// Check all agentic_ssh hooks in settings.
...
/// Check a single hook event for a agentic_ssh entry.
...
fn doctor_check_single_hook(dc: &mut DoctorCounters, settings: &serde_json::Value, event: &str) {
    let Some((bin, sub, is_legacy)) = find_agentic_ssh_hook(settings, event) else {
    ...
    if let Some(expected_matcher) = expected_hook_matcher(event) {
        let actual = find_agentic_ssh_hook_matcher(settings, event);
    ...
    if Path::new(&bin).exists() {
        ...
        dc.fail(&format!(
            "Hook binary not found: {bin} — run `agentic_ssh install`"
        ));
    }
}
...
fn doctor_fix_hooks(dc: &mut DoctorCounters, settings_path: &Path, settings: &serde_json::Value) {
    ...
    for event in &["PreToolUse", "UserPromptSubmit", "Stop"] {
        ...
        let current = find_agentic_ssh_hook(&settings, event);
        let matcher_ok = expected_matcher
            .is_none_or(|m| find_agentic_ssh_hook_matcher(&settings, event).as_deref() == Some(m));
        let correct = current
            .as_ref()
...
fn doctor_check_permissions(dc: &mut DoctorCounters, settings: &serde_json::Value) {
    ...
    if missing.is_empty() {
        ...
        dc.fail(&format!(
            "{} tool permission(s) missing — run `agentic_ssh install`",
            missing.len()
        ));
        for perm in &missing {
    ...
    let stale: Vec<&&str> = installed
        .iter()
        .filter(|p| p.starts_with("mcp__agentic_ssh__") && !expected.contains(&p.to_string()))
        .collect();
    if !stale.is_empty() {
        dc.warn(&format!(
...
/// Check CLAUDE.md contains agentic_ssh rules.
fn doctor_check_claude_md(dc: &mut DoctorCounters, home: &Path) {
    ...
    if claude_md_path.exists() {
        let has_rules = std::fs::read_to_string(&claude_md_path)
            ...
            .contains("agentic_ssh");
        if has_rules {
            dc.pass("CLAUDE.md contains agentic_ssh rules");
        } else {
            dc.fail("CLAUDE.md missing agentic_ssh rules — run `agentic_ssh install`");
        }
    } else {
...
fn doctor_check_local_config(dc: &mut DoctorCounters, project_path: &Path) {
    ...
    if !local_cleaned && !mcp_json_path.exists() && !local_settings_path.exists() {
        ...
        dc.pass("Project-local agentic_ssh install is valid");
    }
}

/// Report a local .mcp.json that registers agentic_ssh. With --local now a
/// supported install mode, this is a valid state — never remove it. Returns
/// true if a agentic_ssh entry was found (so the caller can adjust messaging).
fn doctor_check_local_mcp_json(dc: &mut DoctorCounters, mcp_json_path: &Path) -> bool {
    ...
    if !mcp_val["mcpServers"]["agentic_ssh"].is_object() {
        dc.pass("No agentic_ssh in .mcp.json");
        return false;
    }
    dc.pass(&format!(
        "Local (project-scoped) agentic_ssh install detected in {}",
        mcp_json_path.display()
    ));
    true
}

/// Report a local settings.local.json that references agentic_ssh. Valid under
/// --local; never removed by doctor. Returns true if agentic_ssh was found.
fn doctor_check_local_settings(dc: &mut DoctorCounters, local_settings_path: &Path) -> bool {
    let Ok(contents) = std::fs::read_to_string(local_settings_path) else {
        return false;
    };
    if !contents.contains("agentic_ssh") {
        dc.pass("No agentic_ssh in .claude/settings.local.json");
        return false;
    }
    dc.pass(&format!(
        "Local (project-scoped) agentic_ssh config detected in {}",
        local_settings_path.display()
    ));
    true
}
...
/// Emit a warning if the current agentic_ssh version expects tool permissions
...
fn warn_missing_permissions(settings: &serde_json::Value) {
    ...
    if missing_count > 0 {
        eprintln!(
            "\x1b[33mwarning: {missing_count} new agentic_ssh tool(s) not yet permitted. Run `agentic_ssh reinstall` to update permissions.\x1b[0m"
        );
    }
}

/// Load `path`, normalize any backslashed agentic_ssh hook commands, backfill
...
/// no agentic_ssh hook is present in the file.
fn normalize_and_backfill_settings_file(path: &Path) {
    ...
    // Only touch files that already reference agentic_ssh — don't accidentally
    // rewrite unrelated project settings just because agentic_ssh ran in cwd.
    let Some(bin) = extract_agentic_ssh_bin_from_hooks(&settings) else {
        return;
    };
...
/// Rewrite any agentic_ssh hook command containing a backslash to use forward
/// slashes. Fixes pre-v4.0.x Windows installs where backslashed paths got
/// mangled by `bash -c` (e.g. `C:\Users\...` → `C:Users...` — see issue #38).
/// Only touches commands that mention `agentic_ssh` so unrelated hooks are left
/// alone.
    ...
    for entries in hooks.values_mut() {
        ...
        for entry in arr.iter_mut() {
            ...
            for cmd in cmds.iter_mut() {
                ...
                if command.contains("agentic_ssh") && command.contains('\\') {
...
/// Extracts the agentic_ssh binary path from any existing hook command.
///
/// Scans all hook events for a command containing "agentic_ssh" and returns
/// the binary path. Handles both the modern `{command, args}` shape and the
/// legacy single-string shape. Returns `None` if no agentic_ssh hook is found.
fn extract_agentic_ssh_bin_from_hooks(settings: &serde_json::Value) -> Option<String> {
...
mod tests {
    ...
    /// Build a settings value with the three agentic_ssh hooks installed
    ...
    fn settings_with_all_hooks(bin: &str) -> serde_json::Value {
        json!({
            ...
            "permissions": {
                "allow": ["mcp__agentic_ssh__search", "mcp__agentic_ssh__lookup"]
            }
        })
    }
    ...
    fn uninstall_hook_removes_all_three_events() {
        let mut settings = settings_with_all_hooks("/usr/bin/agentic_ssh");
    ...
    fn uninstall_hook_removes_user_prompt_submit() {
        let mut settings = json!({
            "hooks": {
                "UserPromptSubmit": [{
                    "hooks": [{ "type": "command", "command": "agentic_ssh hook-prompt-submit" }]
                }]
            }
        });
    ...
    #[test]
    fn uninstall_preserves_non_agentic_ssh_hooks() {
        let mut settings = json!({
            "hooks": {
                "UserPromptSubmit": [
                    {
        ...
        // The non-agentic_ssh UserPromptSubmit entry should survive.
        ...
        assert!(arr[0]["hooks"][0]["command"]
            ...
            .unwrap()
            .contains("other-tool"));
        // The Stop event (no agentic_ssh) should survive.
    ...
    #[test]
    fn uninstall_permissions_removes_agentic_ssh_entries() {
        let mut settings = json!({
            "permissions": {
                "allow": [
                    "Bash",
                    "mcp__agentic_ssh__search",
                    "mcp__agentic_ssh__lookup",
                    "Read"
                ]
            }
        });
    ...
    fn install_adds_all_three_hooks() {
        let mut settings = json!({});
        install_hook(&mut settings, "/usr/bin/agentic_ssh");
    ...
    fn install_uses_args_array_for_paths_with_spaces() {
        let bin = "C:/Path With Spaces/agentic_ssh.exe";
        let mut settings = json!({});
    ...
    fn install_is_idempotent_for_legacy_shape() {
        ...
        let mut settings = settings_with_legacy_hooks("/usr/bin/agentic_ssh");
    ...
    fn doctor_repairs_legacy_shape_to_args_array() {
        let legacy_bin = "C:/Path With Spaces/agentic_ssh.exe";
    ...
    // -----------------------------------------------------------------------
    // extract_agentic_ssh_bin_from_hooks tests
    // -----------------------------------------------------------------------
    ...
    fn extract_bin_from_any_hook_event() {
        let settings = json!({
            "hooks": {
                "Stop": [{
                    "hooks": [{ "type": "command", "command": "/opt/bin/agentic_ssh hook-stop" }]
                }]
            }
        });
        assert_eq!(
            extract_agentic_ssh_bin_from_hooks(&settings),
            Some("/opt/bin/agentic_ssh".to_string())
        );
    }
    ...
    fn extract_bin_returns_none_without_hooks() {
        ...
        assert_eq!(extract_agentic_ssh_bin_from_hooks(&settings), None);
    }
    ...
    fn extract_bin_normalizes_windows_backslashes() {
        let settings = json!({
            "hooks": {
                "UserPromptSubmit": [{
                    "hooks": [{ "type": "command", "command": "C:\\Users\\dev\\scoop\\shims\\agentic_ssh.exe hook-prompt-submit" }]
                }]
            }
        });
        assert_eq!(
            extract_agentic_ssh_bin_from_hooks(&settings),
            Some("C:/Users/dev/scoop/shims/agentic_ssh.exe".to_string())
        );
    }
    ...
    #[test]
    fn normalize_rewrites_backslashed_agentic_ssh_commands() {
        let mut settings = json!({
            "hooks": {
                "Stop": [{
                    "hooks": [{
                        "type": "command",
                        "command": "C:\\Users\\alkam\\scoop\\apps\\agentic_ssh\\current\\agentic_ssh.exe hook-stop"
                    }]
                }]
            }
        });
        ...
        assert_eq!(
            settings["hooks"]["Stop"][0]["hooks"][0]["command"]
                .as_str()
                .unwrap(),
            "C:/Users/alkam/scoop/apps/agentic_ssh/current/agentic_ssh.exe hook-stop"
        );
    }

    #[test]
    fn normalize_leaves_non_agentic_ssh_hooks_alone() {
        let mut settings = json!({
            "hooks": {
                "Stop": [{
                    "hooks": [{
    ...
    fn normalize_is_noop_when_already_forward_slashed() {
        let mut settings = settings_with_all_hooks("C:/Users/dev/scoop/shims/agentic_ssh.exe");
        let before = settings.clone();
        normalize_hook_command_paths(&mut settings);
        assert_eq!(settings, before);
    }
  ...
  "hooks": {
    "Stop": [{
      "hooks": [{ "type": "command", "command": "C:\\Users\\u\\agentic_ssh.exe hook-stop" }]
    }]
  }
}
"#;
        ...
        assert_eq!(
            parsed["hooks"]["Stop"][0]["hooks"][0]["command"]
                .as_str()
                .unwrap(),
            "C:/Users/u/agentic_ssh.exe hook-stop"
        );
    ...
    #[test]
    fn normalize_and_backfill_skips_file_without_agentic_ssh_hook() {
        use std::io::Write as _;
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("settings.json");
...
"#;
        ...
        assert_eq!(
            after, contents,
            "file without agentic_ssh hook must be untouched"
        );
    }
    ...
    fn doctor_detects_missing_user_prompt_submit() {
        ...
        let settings = json!({
            "hooks": {
                "PreToolUse": [{
                    "hooks": [{ "type": "command", "command": "agentic_ssh hook-pre-tool-use" }]
                }]
            }
        });
    ...
    fn doctor_detects_wrong_subcommand() {
        ...
        let settings = json!({
            "hooks": {
                "UserPromptSubmit": [{
                    "hooks": [{ "type": "command", "command": "agentic_ssh invalidcommand" }]
                }]
            }
        });
        doctor_check_single_hook(&mut dc, &settings, "UserPromptSubmit");
    ...
    fn doctor_detects_missing_subcommand() {
        ...
        let settings = json!({
            "hooks": {
                "UserPromptSubmit": [{
                    "hooks": [{ "type": "command", "command": "agentic_ssh" }]
                }]
            }
        });
        doctor_check_single_hook(&mut dc, &settings, "UserPromptSubmit");
    ...
    fn doctor_fix_adds_missing_hooks() {
        ...
        let settings = json!({
            "hooks": {
                "Stop": [{
                    "hooks": [{ "type": "command", "command": "/usr/bin/agentic_ssh hook-stop" }]
                }]
            }
        });
        std::fs::write(
    ...
    fn doctor_fix_replaces_wrong_subcommand() {
        ...
        assert_eq!(inner["command"].as_str().unwrap(), "/usr/bin/agentic_ssh");
    }
    ...
    fn doctor_fix_noop_when_all_present() {
        let dir = tempfile::tempdir().unwrap();
        let settings_path = dir.path().join("settings.json");
        let settings = settings_with_all_hooks("/usr/bin/agentic_ssh");
