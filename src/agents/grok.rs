...
fn install_prompt_rules(agents_md: &Path) -> Result<()> {
    ...
    write!(
        ...
        sub-agents (especially the `explore` sub-agent), use the agentic_ssh MCP tools. \
        Discover them with the built-in `search_tool` (query for \"agentic_ssh\" or a code \
        concept) and invoke via `use_tool` with the namespaced name \
        (e.g. `agentic_ssh__agentic_ssh_context`, `agentic_ssh__agentic_ssh_search`, \
        `agentic_ssh__agentic_ssh_callers`, `agentic_ssh__agentic_ssh_callees`, \
        `agentic_ssh__agentic_ssh_impact`, `agentic_ssh__agentic_ssh_node`, \
        `agentic_ssh__agentic_ssh_files`, `agentic_ssh__agentic_ssh_affected`).\n\n\
