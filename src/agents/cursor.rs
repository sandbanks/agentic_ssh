...
//!
//! Handles registration of the agentic_ssh MCP server in Cursor's
//! `~/.cursor/mcp.json` under the `mcpServers.agentic_ssh` key.
...
impl AgentIntegration for CursorIntegration {
    ...
    fn install(&self, ctx: &InstallContext) -> Result<()> {
        ...
        eprintln!("  2. Restart Cursor — agentic_ssh tools are now available");
        Ok(())
    }
    ...
    fn uninstall(&self, ctx: &InstallContext) -> Result<()> {
        ...
        eprintln!();
        eprintln!("Uninstall complete. AgenticSsh has been removed from Cursor.");
...
/// Check ~/.cursor/mcp.json has agentic_ssh MCP server registered.
fn doctor_check_settings(dc: &mut DoctorCounters, home: &Path) {
    let mcp_path = home.join(".cursor/mcp.json");

    if !mcp_path.exists() {
        dc.warn(&format!(
            "{} not found — run `agentic_ssh install --agent cursor` if you use Cursor",
            mcp_path.display()
        ));
        return;
    }
    ...
    if server.and_then(|v| v.as_object()).is_some() {
        ...
        dc.fail(&format!(
            "MCP server NOT registered in {} — run `agentic_ssh install --agent cursor`",
            mcp_path.display()
        ));
    }
}
