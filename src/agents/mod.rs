...
/// A CLI agent that can be configured to use agentic_ssh via MCP.
pub trait AgentIntegration {
    ...
    /// Returns true if agentic_ssh MCP server is already registered in this
    /// agent's config. Used for migration backfill.
    fn has_agentic_ssh(&self, _home: &Path) -> bool {
        false
    }
...
/// Use this for **read-only** paths (healthcheck, `has_agentic_ssh`, etc.).
...
pub fn load_json_file_strict(path: &Path) -> Result<serde_json::Value> {
    ...
    let contents = std::fs::read_to_string(path).map_err(|e| AgenticSshError::Config {
        message: format!("cannot read {}: {e}", path.display()),
    })?;
    if contents.trim().is_empty() {
        return Ok(serde_json::json!({}));
    }
    serde_json::from_str(&contents).map_err(|e| AgenticSshError::Config {
        message: format!(
...
pub fn backup_config_file(path: &Path) -> Result<Option<PathBuf>> {
    ...
    let content = std::fs::read(path).map_err(|e| AgenticSshError::Config {
        message: format!(
...
pub fn safe_write_json_file(
    ...
    // 1. Serialize
    let pretty = serde_json::to_string_pretty(value).map_err(|e| AgenticSshError::Config {
    ...
    if serde_json::from_str::<serde_json::Value>(&pretty).is_err() {
        return Err(AgenticSshError::Config {
            message: format!(
                "internal error: serialized JSON for {} failed re-parse validation.\n  \
                 This is a bug in agentic_ssh — please report it.",
                path.display()
            ),
        });
    }
...
/// Finds the agentic_ssh binary path.
///
...
pub fn load_jsonc_file_strict(path: &Path) -> Result<serde_json::Value> {
    ...
    serde_json::from_str(&stripped).map_err(|e| AgenticSshError::Config {
        message: format!(
...
/// Returns agent IDs that have agentic_ssh configured under `home` but are
...
pub fn detect_missing_installed_agents(home: &Path, current: &[String]) -> Vec<String> {
    ...
    for ag in all_integrations() {
        ...
        if ag.has_agentic_ssh(home) && !current.contains(&id) {
            additions.push(id);
        }
    }
    additions
...
mod migrate_tests {
    ...
    /// Writes a minimal `~/.claude.json` so `ClaudeIntegration::has_agentic_ssh`
    ...
    /// Regression test for the bug where `agentic_ssh reinstall` skipped Claude
...
pub fn pick_integrations_interactive(
    ...
    for ag in &detected {
        ...
        if already {
            eprint!("Keep agentic_ssh for {}? [Y/n] ", ag.name());
        } else {
            eprint!("Install agentic_ssh for {}? [Y/n] ", ag.name());
        }
...
/// but cannot be parsed as a TOML document, returns a [`AgenticSshError::Config`]
...
pub fn load_toml_file(path: &Path) -> Result<toml::Value> {
    ...
    let table: toml::Table = toml::from_str(&contents).map_err(|e| AgenticSshError::Config {
        message: format!(
...
pub fn write_toml_file(path: &Path, value: &toml::Value) -> Result<()> {
    ...
    std::fs::write(path, contents).map_err(|e| AgenticSshError::Config {
...
/// Whether `agentic_ssh install` should install the global git `post-commit`
...
/// The marker comment used to identify agentic_ssh's section in a hook script.
const HOOK_MARKER: &str = "# agentic_ssh: auto-sync";

/// The hook snippet appended to (or written as) the post-commit script.
fn post_commit_snippet(agentic_ssh_bin: &str) -> String {
    let bin = agentic_ssh_bin.replace('\\', "/");
    format!(
        "{HOOK_MARKER}\n\
...
/// If a global git `post-commit` hook is not already set up for agentic_ssh,
...
pub fn offer_git_post_commit_hook(agentic_ssh_bin: &str, mode: GitHookMode) {
    ...
    match decide_hook_action(mode, existing_contents.as_deref()) {
        HookAction::AlreadyInstalled => {
            eprintln!("  Global git post-commit hook already contains agentic_ssh, skipping");
            return;
        }
        HookAction::Skip => {
    ...
    if matches!(mode, GitHookMode::Default) {
        eprintln!();
        eprint!(
            "Install a global git post-commit hook to auto-run \x1b[1magentic_ssh sync\x1b[0m after each commit? [y/N] "
        );
    ...
    let snippet = post_commit_snippet(agentic_ssh_bin);
...
mod git_hook_tests {
    ...
    fn decide_hook_action_yes_reports_already_installed_when_marker_present() {
        let contents = "#!/bin/sh\n# agentic_ssh: auto-sync\n/usr/bin/agentic_ssh sync\n";
        assert_eq!(
            decide_hook_action(GitHookMode::Yes, Some(contents)),
            HookAction::AlreadyInstalled
        );
    ...
    fn decide_hook_action_no_still_reports_already_installed() {
        ...
        let contents = "# agentic_ssh: auto-sync\nfoo\n";
        assert_eq!(
...
pub fn tool_names() -> Vec<String> {
    vec![
        ...
        "wait_for_log_pattern".to_string(),
...
pub fn expected_tool_perms() -> Vec<String> {
    tool_names()
        .into_iter()
        .map(|name| format!("mcp__agentic_ssh__{}", name))
        .collect()
}
...
mod safe_config_tests {
    ...
    // returned {} on parse failure, and the install wrote {} + agentic_ssh
    ...
    fn full_install_cycle_preserves_existing_config() {
        ...
        config["mcp"]["agentic_ssh"] = serde_json::json!({
            "type": "local",
            "command": ["agentic_ssh", "serve"]
        });
        ...
        let result: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&path).unwrap()).unwrap();
        // AgenticSsh was added
        assert!(result["mcp"]["agentic_ssh"].is_object());
        ...
        assert!(bak_content.get("agentic_ssh").is_none());
...
mod path_normalize_tests {
    ...
    fn normalizes_windows_backslashes() {
        assert_eq!(
            normalize_path_separators(r"C:\Users\dev\scoop\shims\agentic_ssh.exe"),
            "C:/Users/dev/scoop/shims/agentic_ssh.exe"
        );
    }
    ...
    fn leaves_unix_paths_unchanged() {
        assert_eq!(
            normalize_path_separators("/usr/local/bin/agentic_ssh"),
            "/usr/local/bin/agentic_ssh"
        );
    }
}
...
mod install_scope_tests {
    ...
    fn install_context_base_dir_follows_scope() {
        ...
        let global = InstallContext {
            home: home.clone(),
            agentic_ssh_bin: "agentic_ssh".into(),
